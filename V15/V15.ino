...
#define humedad_vege_pin 2   // Pin del sensor de humedad de Vegetacion
#define humedad_flora_pin 0  // Pin del sensor de humedad de Flora
#define Lm35pin 4            // Pin del sensor de temperatura LM35DZ
#define dht_22 16            // Pin del sensor de temperatura y humedad DHT22
#define sensormq9 17         // Define el pin analógico donde está conectado el sensor MQ-9
// Caudalimetro
#define Caudalimetro_uno 12
#define Caudalimetro_dos 14
#define Caudalimetro_tres 27
// Boton
...
#define luzpin 35          // Pin del relé 4 (Iluminación)
#define ventilacionpin 32  // Pin del relé 5 (Ventilación)
...
float humedadSuelo_vege = 0;   // Guardar datos de la humedad del suelo Vegetacion
float humedadSuelo_flora = 0;  // Guardar datos de la humedad del suelo Flora
...
void setup() {
  ...
  //Caudalimetro
  pinMode(Caudalimetro_uno, INPUT);
  pinMode(Caudalimetro_dos, INPUT);
  pinMode(Caudalimetro_tres, INPUT);
...
/*void pagina_web() {
  ...
  server.on("/", HTTP_GET, []() {
    ...
    html += "<p>Humedad del suelo Sensor de la etapa Crecimiento: ";
    //html += String(ThingSpeak.readFloatField(channelID, 1));
    html += String(analogRead(humedad_pin));
    html += "<p>Estado de la bomba de la etapa Crecimiento: ";
    if (digitalRead(bombapin) == 1) {
      html += "Bomba activa ";
    } else {
      html += "Bomba apagada ";;
    }   
    html += "<p>Humedad del suelo Sensor de la etapa Vegetacion: ";
    html += String(analogRead(humedad_vege_pin));
    html += "<p>Estado de la bomba de la etapa Vegetacion: ";
    if (digitalRead(bombadospin) == 1) {
      html += "Bomba activa ";
    } else {
      html += "Bomba apagada ";;
    }   
    html += "<p>Humedad del suelo Sensor de la etapa Flora: ";
    html += String(analogRead(humedad_flora_pin));
    html += "<p>Estado de la bomba de la etapa Flora: ";
...
void pagina_web() {
  ...
  ptr +=String(analogRead(humedad_pin));
  if (digitalRead(bombapin) == 1) {
  ...
  ptr +="<div class='side-by-side reading'>";
  ptr +=String(analogRead(humedad_vege_pin));
  if (digitalRead(bombapin) == 1) {
      ptr += "<h3>Bomba activa </h3>";
    } else {
      ptr += "<h3>Bomba apagada </h3>";;
  ...
  ptr +="<div class='side-by-side reading'>";
  //ptr +=String(bmp.readTemperature());;
  ptr +="<span class='superscript'>&deg;C</span></div>";
  ptr +="</div>";
  ...
  ptr +="<div class='side-by-side reading'>";
  ptr +=String((analogRead(Lm35pin) * 3.3) / 4095.0);
  ptr +="<span class='superscript'>&deg;C</span></div>";
  ptr +="</div>";
  ...
  ptr +="<div class='side-by-side reading'>";
  //ptr +=String(bmp.readPressure() / 100.0F);;
  ...
  ptr +="<div class='side-by-side reading'>";
  //ptr +=String(bmp.readAltitude(1013.25));
...
void humedad_del_suelo() {
  ...
  humedadSuelo_vege = humedadSuelo_dos;
  // Flora
  float humedadSuelo_tres = analogRead(humedad_flora_pin);
  humedadSuelo_flora = humedadSuelo_tres;

  Serial.print("H. Inicio: ");
  Serial.println(humedadSuelo);
  Serial.print("H. Vegetacion: ");
  Serial.println(humedadSuelo_vege);
  Serial.print("H. Flora: ");
  Serial.println(humedadSuelo_flora);
}
...
void humedad_suelo() {
  ...
  if (humedadSuelo_vege < menor_porcentaje_vege) {
    digitalWrite(bombadospin, LOW);
  } else if (humedadSuelo_vege > mayor_porcentaje_vege) {
    digitalWrite(bombadospin, HIGH);
  }
  if (humedadSuelo_flora < menor_porcentaje_flora) {
    digitalWrite(bombatrespin, LOW);
  } else if (humedadSuelo_flora > mayor_porcentaje_flora) {
...
void paginaweb() {
  ...
  ThingSpeak.setField(2, humedadSuelo_vege);
  ThingSpeak.setField(3, humedadSuelo_flora);
  ThingSpeak.setField(4, temperatura);   //LM35
  ThingSpeak.setField(5, temperaturaD);  //DHT22
  ThingSpeak.setField(6, humedadD);      //DHT22
  ThingSpeak.setField(7, ppm);           //CO2
...
void Adafruit_IO_loop() {
  myFeed->save(humedadSuelo);
  myFeed->save(humedadSuelo_vege);
  myFeed->save(humedadSuelo_flora);
  myFeed->save(temperatura);
  myFeed->save(temperaturaD);
  myFeed->save(humedadD);
  myFeed->save(ppm);
}